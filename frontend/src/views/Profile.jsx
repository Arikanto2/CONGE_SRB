...
export default function Profile() {
  ...
  useEffect(() => {
    const fetchCongeAnnuel = async () => {
      try {
        ...
        console.log("Erreur lors de la récupération des congés annuels");
        if (error.response) {
  ...
  const submitFormMDP = async () => {
    ...
    try {
      ...
      await axios.post("http://localhost:8000/api/change-password", donneeMDP);
  ...
  const submitForm = async () => {
    ...
    try {
      ...
      // Parcourir les données et traiter PHOTO_PROFIL séparément
      for (const key in donneepers) {
        if (key === "PHOTO_PROFIL") {
          // N'ajouter PHOTO_PROFIL que si c'est un fichier
          if (donneepers[key] instanceof File) {
